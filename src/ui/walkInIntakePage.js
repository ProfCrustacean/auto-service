...
function buildPrefillHref(values, overrides = {}) {
  ...
  return serialized.length > 0 ? `/intake/walk-in?${serialized}` : "/intake/walk-in";
}
...
function renderSummaryCard({ title, content }) {
  return `<div class="summary-card"><strong>${escapeHtml(title)}</strong><span>${escapeHtml(content)}</span></div>`;
}
...
<body>
  <div class="wrap">
    ...
    <section class="panel row">
      ...
      <form class="lookup-form" method="get" action="/intake/walk-in">
        ...
        ${String(values.q).trim().length > 0 ? '<a class="btn" href="/intake/walk-in">Сбросить</a>' : ""}
      </form>
